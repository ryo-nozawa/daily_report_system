...
public abstract class ActionBase {
     ...
     * context  Webアプリケーションのコンテキスト情報
     * request リクエスト情報のオブジェクト
     * response    レスポンス情報のオブジェクト
     *
    ...
    /**
     * フロントコントローラから呼び出されるメソッド=サブクラスで内容を定義
    ...
    protected void invoke() throws ServletException, IOException {
        ...
        try {
            /*ForwardConstからパラメータで送られたメソッド文をcommandに格納
             * thisが示すクラス内で定義されたcommand（メソッド文）を引数なしのメソッドでcommandMethodに格納
             * commandMethodに格納したメソッドを実行
            ...
            e.printStackTrace();  //発生した例外をコンソールに表示
            forward(ForwardConst.FW_ERR_UNKNOWN);   //commandの値が不正で実行できない場合エラー画面を呼び出し
        }
    }

    /**
     * 指定されたjspの呼び出しを行う　Get文のrequestdispatcher
    ...
    protected int toNumber(String strNumber) {
        ...
        try {
            ...
            number = Integer.MIN_VALUE; //数値型変数の最小値
        }
        return number;
    }
